...
/// These two are also a published contract, in `vibe_ipc_proto` — a policy manifest declares
...
const _: () = {
    assert!(OBS_LEN == vibe_ipc_proto::POLICY_OBS_LEN);
    assert!(ACTION_LEN == vibe_ipc_proto::POLICY_ACTION_LEN);
};
